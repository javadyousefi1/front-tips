...
// Start calculation with Web API Worker
startWorkerBtn.addEventListener('click', () => {
    ...
    // Update UI
    updateStatus('Calculating with Web API Worker...', 'running');
    outputDiv.textContent = 'Processing...';
});

// Start calculation WITHOUT Web API Worker (blocks UI)
startNoWorkerBtn.addEventListener('click', () => {
    ...
    updateStatus('Calculating WITHOUT Web API Worker (UI will freeze)...', 'running');
...
function handleComplete(primes, time, usedWorker) {
    const method = usedWorker ? 'Web API Worker' : 'Main Thread';
...
outputDiv.textContent = 'Click "Start with Worker" to see Web API Workers in action!\n\nTry interacting with the slider above during calculation to see the difference between using and not using a Web API Worker.';
